...
#pragma region DirectX11Init
	...
	hr = m_pDevice->CreateRasterizerState(&hRasterizerDesc, &m_RasterStateWire);
	if (FAILED(hr)) {
		return hr;
	}
...
#pragma region ImGui
	...
	if (!ImGui_ImplDX11_Init(DirectX11Manager::m_pDevice.Get(), DirectX11Manager::m_pImContext.Get()))
	{
...
void DirectX11Manager::TurnSolid()
{
	m_pImContext->RSSetState(m_pRasterizerState.Get());
}

void DirectX11Manager::TurnWire()
{
	m_pImContext->RSSetState(m_RasterStateWire.Get());
}
