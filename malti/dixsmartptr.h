...
// スマートポインタテンプレート  v2.26
//  Created by IKD (2017. 9. 23) 
...
// 2017. 9. 23( v2.26 )
// 単体と配列のウィークポインタをSwapPtrメソッドで交換するとデストラクタが呼ばれ続けるバグを修正
...
namespace Dix {
	...
	//■■■演算子のオーバーロード■■■■■■■■■■■
		...
		T& operator[]( int elem ) const {
			return IsAry() ? (*m_ppPtr)[ elem ] : ( *m_ppPtr )[ 0 ];
		}
		...
		T* operator+( int add ) const {
			return IsAry() ? ( *m_ppPtr + add ) : *m_ppPtr;
		}
	...
	public:
	   ...
	   /////
		...
		template <class T2> bool DownCast( sp<T2> &src )
		{
			...
			// 引数が配列の場合は全要素をチェックできないのでダウンキャストしない
			if ( src.IsAry() == true )
				return false;
		...
		bool SwapPtr( sp<T> &src )
		{
			...
			*m_ppPtr	 = pTmp;
			...
			// 配列フラグの交換
			bool* tmp	 = src.getIsAryPtr();
			src.m_pIsAry = m_pIsAry;
			m_pIsAry	 = tmp;
		...
		///////
		bool IsAry() const {
			return m_pIsAry ? *m_pIsAry : false;
		}
	...
	//■■■演算子のオーバーロード■■■■■■■■■■■
		...
		template< class T2 > bool operator ==( sp<T2>& val ) const {
			if ( m_ppPtr != 0 && *m_ppPtr == val.GetPtr() )
				return true;
			if ( m_ppPtr == 0 && val.GetPtr() == 0 )
				return true;
		...
		template< class T2 > bool operator ==( wp<T2>& val ) const {
			if ( m_ppPtr != 0 && *m_ppPtr == val.GetPtr() )
				return true;
			if ( m_ppPtr == 0 && val.GetPtr() == 0 )
				return true;
	   ...
	   /////
		...
		bool SwapPtr( wp<T> &src )
		{
			...
			*m_ppPtr = pTmp;
			...
			// 配列フラグの交換
			// spのフラグを直接変更
			bool* pSrcFlag = src.getIsAryPtr();
			bool srcFlag = *pSrcFlag;	// 相手のフラグ
			bool myFlag = *m_pIsAry;	// 自分のフラグ
			*pSrcFlag = myFlag;
			*m_pIsAry = srcFlag;
